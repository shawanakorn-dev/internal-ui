...
-- GUI functions and objects
local function createGuiObject(className)
	return {
		Name = "",
		ClassName = className,
		Parent = nil,
		...
		Clone = function(self) return createGuiObject(self.ClassName) end,
		Destroy = function() end
	}
end

-- Basic Roblox classes
Instance = {
	new = function(className)
		return createGuiObject(className)
	end
}
...
UDim2 = {
	new = function(x, y)
		return {
			X = { Scale = x or 0, Offset = 0 },
			Y = { Scale = y or 0, Offset = 0 },
...
-- Add gethui function if not available
if not gethui then
	function gethui()
		return {
			RobloxGui = createGuiObject("ScreenGui")
		}
	end
end

-- Basic Roblox functions
...
if not task then
	task = {
		...
		wait = function(t) return os.execute("sleep " .. (t or 0)) end,
...
-- Basic filesystem functions if not available
...
function game:GetService(serviceName)
	return services[serviceName] or {}
end

_G.game = game

-- Allow direct execution for development
local USING_JALON_LOADER = true  -- Set this to true by default

-- Add import function for direct execution
local function import(path)
	if type(path) ~= "string" then return nil end
	
	-- Handle src directory
	if path:sub(1, 4) == "src/" then
		path = path:sub(5)  -- Remove "src/" prefix
	end
